...
func (s *StatsService) build(ctx context.Context) (*model.Statistics, error) {
	...
	// 任务：用单次加锁的快照取 total/running/listTotal，保证三者同源、
	// overview 的 task_count 与任务列表 total 严格一致。
	tCount, rCount, tListTotal := s.stores.Tasks.TaskCountSnapshot()
	_ = tListTotal

	// 设备：用单次加锁的快照取 total/online/listTotal，保证 overview 的
	// device_count / online_count 与设备列表 total 严格一致。
	devTotal, oCount, dListTotal := s.stores.Devices.DeviceCountSnapshot()
	_ = dListTotal

	// 计数器即真相源，快照下三者同源，仅做逻辑上的保护性 clamp。
	finalTask := tCount
	if finalTask < 0 {
		finalTask = 0
	}
	...
	if finalRunning > finalTask {
		finalRunning = finalTask
	}
	finalDevice := devTotal
	if finalDevice < 0 {
		finalDevice = 0
	}
	finalOnline := oCount
